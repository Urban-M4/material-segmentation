        ...
        if self.param:
            if self.AxTrans:
                x = (x - x.mean(dim=-1, keepdim=True)) / (
                    x.std(dim=-1, keepdim=True) + 1e-12
                )
...
class SAMixer(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        # MHSA
        box0 = self.norm0(
            self.avg_local1(x).view(b, 1, c, N).permute(0, 3, 1, 2).reshape(B, 1, c)
        )
        q = (
            self.q(box0).view(B, 1, g, d).transpose(1, 2) * self.scl
        )  # only x serves as the q, [B, g, 1, d]
        kv0 = (
            self.kv(box0).view(B, 1, 2, g, d).permute(2, 0, 3, 1, 4)
        )  # [2, B, g, 1, d]

        kv1 = (
            F.interpolate(
                self.kv(
                    self.norm1(x1.view(b, c, N // 16).transpose(1, -1).contiguous())
                )
                .transpose(1, -1)
                .reshape(b, 2 * c, h, w),
                scale_factor=4,
        ...
        kv3 = (
            self.kv(
                self.norm3(x3.view(b, 1, c, N).permute(0, 3, 1, 2).reshape(B, 1, c))
            )
            .view(B, 1, 2, g, d)
            .permute(2, 0, 3, 1, 4)
        )  # [2, B, g, 1, d]
        ...
        u_attn = (
            (sim @ v).transpose(1, 2).reshape(B, 1, c)
        )  # b g i j, b g j c -> b g i c (i=1, j=4)
        ...
        x = self.proj_top(
            self.combine_add(
                x,
                self.norm_outer(
                    self.mlp(self.norm_inner(x)).flatten(2).transpose(1, -1)
                )
                .transpose(1, -1)
                .reshape(b, c, H * T, W * T),
            )
...
class SAMixerHead(nn.Module):
    ...
    def __init__(
        ...
        self.mixer = SAMixer(
            dim=embedding_dim, rate_reduct=1, branches=4, norm_layer=norm_layer
        )
