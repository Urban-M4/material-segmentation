...
class TrainerAdv(object):
    def __init__(self, args):
        ...
        optimizer = torch.optim.AdamW(
            train_params, betas=(0.9, 0.999), weight_decay=0.01
        )
        ...
        # Define Criterion
        self.criterion = SegmentationLosses(weight=None, cuda=args.cuda).build_loss(
            mode=args.loss_type
        )
        ...
        train_params = [
            {"params": self.model.backbone.parameters(), "lr": args.lr},
    ...
    def training(self, epoch):
        ...
        print(
            ...
            % (epoch, i * self.args.batch_size + image_cat.data.shape[0])
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "kitti",
            "kitti_advanced",
            "kitti_advanced_manta",
            "handmade_dataset",
            "handmade_dataset_stereo",
        ],
    ...
    parser.add_argument(
        "--use-aolp", action="store_true", default=False, help="use aolp"
    )
    parser.add_argument(
        "--use-dolp", action="store_true", default=False, help="use dolp"
    )
    parser.add_argument("--use-nir", action="store_true", default=False, help="use nir")
    parser.add_argument(
    ...
    # give weight masks to the objective function (no need for eval phase)
    ...
    trainer.model.load_state_dict(
        torch.load(
            trainer.save_path / "rmsnet_split1.pth", map_location=torch.device("cpu")
        )
    )  # for split-1
    # trainer.model.load_state_dict(
    #     torch.load(
    #         trainer.save_path / "rmsnet_split2.pth", map_location=torch.device("cpu")
    #     )
    # )  # for split-2
